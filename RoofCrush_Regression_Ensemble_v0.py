...
def my_model_fn_2(
    ...
    # with tf.name_scope('Model_2_Layers'):
    with tf.variable_scope('Model_2_Layers'):
        regularizer = tf.keras.regularizers.l2(l=0.01)
        initializer = tf.keras.initializers.glorot_normal(seed=None)
        input_layer = tf.feature_column.input_layer(features, feature_columns)
    ...
    with tf.variable_scope('Model_2_Layers', reuse=True):
        weight_value_1 = tf.get_variable("First_Hidden_Layer/kernel")
        weight_value_2 = tf.get_variable("Second_Hidden_Layer/kernel")
        weight_value_3 = tf.get_variable("Third_Hidden_Layer/kernel")
        bias_value_1 = tf.get_variable("First_Hidden_Layer/bias")
...
classifier = tf.estimator.Estimator(model_fn=my_model_fn, model_dir="./model_1")  # Path to where checkpoints etc are stored

tf.logging.info("2nd MODEL CONSTRUCTION")
classifier_2nd = tf.estimator.Estimator(model_fn=my_model_fn_2, model_dir="./model_2")
